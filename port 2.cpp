...
//call a destroyed at the end of programs
...
string invalidMessage(int a, int b) {
    Buffer_Clear();
    string n;
    n = "\nYou have failed to input a valid number. Please input an interget between " + to_string(a) + " and " + to_string(b) + "...\n";
    return n;
}
string invalidMessage(string a, string b) {
    Buffer_Clear();
    string n;
    n = "\nYou have failed to input a valid number. Please input an interget between " + a + " and " + b + "...\n";
    return n;
}
...
int numValidInput(int boundmax, string inputmessage, int boundmin = 1) {
    int temp;
    do {
    ...
    } while (boundValidation(boundmax, temp, boundmin));
    return temp;
}
...
string cordValidInput(int xybound, int xymin = 1) {
    ...
    do {
        string a = "A1";
        string b = intToAsciiString(xybound) + to_string(xybound);
        string msgQcord = "Where would you like to hide your first boat?\nposition: ";
        std::cin >> input;
        if (std::cin.fail()) {
    ...
    } while (validateCords(input, xybound));
    return input;
}
        ...
        if (totalBoats == 1) {
            ...
            if (isAI) {
                ...
                for (int i = 0; i < totalBoats; i++) {
                    ...
                    string cords = cordValidInput(glength);
                    if (cords == "XX") {
...
Player combat(Player pA, Player pB) {
    ...
    //lose coinflip
        std::cout << pB.getName() << " has won the die-roll!" << endl;
        turns++;
    }
    else {
    //win coinflip
        std::cout << pA.getName() << " has won the die-roll!" << endl;
    }
...
int mainMenu() {
    std::cout << "(1) Play game versus AI" << endl;
